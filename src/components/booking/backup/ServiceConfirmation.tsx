...
export default function ServiceConfirmation({
  ...
  const getSelectedProfessionalForService = (serviceId: string) => {
    ...
    return profissionais.find(p => p.id === serviceProfessional.professionalId);
  };
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 flex flex-col">
          ...
          <div className="space-y-3">
            {getSelectedServices().map((service) => (
              <div key={service!.id} className="border-l-4 border-blue-300 bg-blue-50 p-3 rounded-r-lg">
                ...
                {/* Seleção de profissional compacta - OBRIGATÓRIA */}
                <div className="relative">
                  <button
                    ...
                    className={`
                      flex items-center space-x-2 px-3 py-2 border rounded-lg transition-colors w-full justify-between text-sm
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      <User size={14} className={getSelectedProfessionalForService(service!.id) ? "text-gray-400" : "text-red-400"} />
                      <span className={getSelectedProfessionalForService(service!.id) ? "text-gray-700" : "text-red-700"}>
                        {getSelectedProfessionalForService(service!.id) 
                          ? getSelectedProfessionalForService(service!.id)!.nome.replace('[Exemplo] ', '') 
                          : 'Selecione um colaborador *'}
                      </span>
                    </div>
                    <ChevronDown size={14} className={getSelectedProfessionalForService(service!.id) ? "text-gray-400" : "text-red-400"} />
                  </button>
                              ...
                              <div className="flex items-center space-x-2">
                                <div className="w-5 h-5 bg-purple-100 rounded-full flex items-center justify-center">
                                  <span className="text-purple-600 text-xs font-medium">
                                    {prof.nome.charAt(prof.nome.indexOf(']') + 2).toUpperCase()}
                                  </span>
                                </div>
                                <span className="text-gray-700">{prof.nome.replace('[Exemplo] ', '')}</span>
                              </div>
        ...
        <div className="border-t border-gray-200 bg-white p-4">
          ...
          {/* Mostrar aviso se nem todos os serviços têm profissionais */}
          {!allServicesHaveProfessionals() && (
            <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-xs text-red-700">
                * Selecione um colaborador para cada serviço antes de continuar
              </p>
            </div>
          )}
